...
// Create a pivot group to rotate around the visual center of displaced geometry
const pivotGroup = new THREE.Group();

const sc = s('meshScale', 1);
mesh.scale.set(sc, sc, sc);

// Offset mesh Z so rotation happens around the displaced content's visual center
// With dispScale ~1 and negative Z displacement, center depth is around 0.5
// Offset by -0.5 to place visual center at pivot origin
...
console.log('Mesh added to scene (with pivot group)');
...
const tiltY = Math.max(-35, Math.min(35, e.beta - 35)) / 35; // Subtract 35 for typical phone holding angle
...
// Apply rotation to pivot group (centered rotation)
pivotGroup.rotation.x = target.y;
if (autoRotate && !isHovering) {
pivotGroup.rotation.y += rotSpeed;
}
pivotGroup.rotation.y += (target.x * maxTilt - pivotGroup.rotation.y) * 0.05;
